...
class LoginViewModel: ViewModel() {
    ...
    fun loginWithGoogle(activity : Activity){
        ...
        activity.startActivityForResult(signInIntent, 1)//lanzamos intent
    }

    fun clearErrors() {
        hasGoogleError.postValue(false)
    }
    ...
    fun finishLogin(task: Task<GoogleSignInAccount>) {
        try {
                ...
                auth.signInWithCredential(credential)
                    .addOnCompleteListener {
                        if (it.isSuccessful){
                            ...
                            googleError.postValue("Algo salió mal")
                        }
            ...
            hasGoogleError.postValue(true)
            googleError.postValue("Error de Google")
            isLoading.postValue(false)
